...
이 문제에 최적 부분 기법을 적용하기 위해서는 
1. 입력된 숫자들을 group 짓는다 (각 group의 크기를 결정한다)
2. 각 group을 대표하여 양자화 할 값을 지정해야 한다

### gruop이 주어졌을 떄 양자화 할 값 찾기

```
sum (M - A[i])^2
= sum (M^2 -2A[i]M + A[i]^2)
```
여기서 sum(A[i]) 와 sum(A[i]^2)은 반복 사용되므로 초기에 미리 계산
